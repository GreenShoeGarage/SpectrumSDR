...
- **v0.4.4** Fixed continuous sample loss in the USB streaming loop, diagnosed from hardware telemetry (buffer sawtoothing at 2% with the underrun counter climbing forever). The old pump issued one 16 KB transfer at a time on the main thread; every gap between transfers, and every waterfall-drawing stall, let the RTL2832's small internal FIFO overflow and discard samples, so RF looked fine while audio was fed below real time. The pump now keeps eight 64 KB transfers concurrently in flight, librtlsdr style, reposting each the instant it completes, roughly 109 ms of USB-level cushion. Full-path bench at the new block size: one second of 2.4 MS/s FM through ring, capture hook, and demodulator in 174 ms.
